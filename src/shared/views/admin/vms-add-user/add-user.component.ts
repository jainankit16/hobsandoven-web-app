...
import { UsersApi, ProjectApi, FilterServiceApi, RecordTypeApi, AccountApi } from './../../../sdk';
...
@Component({
    selector: 'app-vmsadd-user',
    templateUrl: './add-user.component.html',
    styleUrls: ['./add-user.component.css']
})

export class VMSAddUserComponent implements OnInit {
    ...
    constructor(
        ...
        if ((this._appState.getAccessType() === 'vendor') ||
            ...
            (this._route.snapshot.data.permissions.indexOf('vendor') !== -1)) {
            this.isVendor = true;
        }
        this.modelList = {
            'account': [],
    ...
    initialLoad() {
        ...
        if (this.viewId && this.viewMode !== 'add') {
    ...
    getDropdownValues() {
        ...
        const paramObj = {};
        paramObj['models'] = ['UserType', 'Country', 'Department', 'Region', 'WorkerShift',
    ...
    setMemberType(hidePreLoader = false) {
    ...
    onEditClick() {
        this._router.navigate(['/vms/user-management/vms-update', this.viewId], { queryParams: { type: this.viewType } });
    }
    ...
    getAccountArr(accounts) {
        if (accounts && accounts.sfdcId && accounts.sfdcId.length) {
            this.currentAccountId = accounts.sfdcId[0];
            this.registrationForm.patchValue({ account_Type: this.currentAccountId });
            this._preloaderService.showPreloader();
            this._accountApi.find(
                {
                    where: { sfdcId: this.currentAccountId }
                }
            ).subscribe(
                res => {
                    if (res && res.length) {
                        this.selectedAccount = res[0];
                        this.currentMemberType = 'vendor';
                        this.setMemberType(true);
                    }
                }, err => {
                    this._alertService.error(err.message);
                    window.scrollTo(0, 0);
            ...
            this._usersApi.getUserList(obj).subscribe(
                res => {
                    if (res && res.length) {
                        ...
                        this.getAccountArr({sfdcId: [this.currentAccountId]});
